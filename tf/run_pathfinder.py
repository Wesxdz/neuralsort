...
# EC Neural Tensor Pathfinder Alpha Test
...
elif method == 'neural_pathfinder':
    scores = multi_mnist_cnn.deepnn(l, X, 1)
    scores = tf.reshape(scores, [M, n, 1])
    P_hat = util.neuralsort(scores, temperature)

    # To train NeuralSort we need P_true
    # however, we want to consider the case where P_true is unknown at training time
    # that is to say, we are trying to sort matrices whose sorting order is unknown by the programmer (ARC)
    # and all we have is a sequence of open set sorts leading to a termination condition
    # such as a successful goal or time limit
    # true_scores prior were MNIST values
    # in this case, true scores are not MNIST values, but the 'optimal' sorting order
    # which may be learned by saving the model's weights as the pathfinder heuristic
...
if should_load_model_from_volume:
    ...
    # We need to run the neural network during the training process
