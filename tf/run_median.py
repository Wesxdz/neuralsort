...
if method == 'vanilla':
    with tf.compat.v1.variable_scope("phi"):
        representations = multi_mnist_cnn.deepnn(l, X, 10)
    representations = tf.reshape(representations, [M, n * 10])
    fc1 = tf.compat.v1.layers.dense(representations, 10, tf.nn.relu)
    fc2 = tf.compat.v1.layers.dense(fc1, 10, tf.nn.relu)
    fc3 = tf.compat.v1.layers.dense(fc2, 10, tf.nn.relu)
    y_hat = tf.compat.v1.layers.dense(fc3, 1)
    y_hat = tf.squeeze(y_hat)
    loss_phi = tf.reduce_sum(tf.math.squared_difference(y_hat, y))
    loss_theta = loss_phi
    prob_median_eval = 0
    ...
    with tf.compat.v1.variable_scope('theta'):
    ...
    exp_loss = tf.math.squared_difference(y, point_estimates)
    ...
    losses = tf.math.squared_difference(
    ...
    exp_loss_eval = tf.math.squared_difference(y, point_estimates)
    ...
    losses = tf.math.squared_difference(regression_candidates, res_y)
...
phi = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='phi')
theta = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='theta')

train_phi = tf.compat.v1.train.AdamOptimizer(
    initial_rate).minimize(loss_phi, var_list=phi)

if method != 'vanilla':
    train_theta = tf.compat.v1.train.AdamOptimizer(initial_rate).minimize(
